...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		Show-InstallationWelcome -CloseApps 'iexplore,netbeans64' -CheckDiskSpace -PersistPrompt

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		If (Test-Path -path "$envProgramFiles\NetBeans 8.2\"){
			Remove-Folder -Path "$envProgramFiles\NetBeans 8.2\"
			...
			Remove-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\nbi-nb-base-8.2.0.0.201609300101-1' -ContinueOnError $True
			Remove-Folder -Path "${envCommonStartMenuPrograms}\NetBeans"
			Remove-MSIApplications -Name "Java 8 Update 151" -PassThru
			Remove-MSIApplications -Name "Java SE Development Kit 8 Update 151" -PassThru
			}



		<#
		$installedApplication = Get-InstalledApplication -Name 'NetBeans'
		If ($installedApplication) {
			If ($installedApplication.DisplayVersion -eq "8.1") {
				[string]$installLocation = $installedApplication.InstallLocation.Trim('"')
				Write-Log -Message "NetBeans was detected in $installLocation and will be removed." -Source $deployAppScriptFriendlyName
				Remove-Folder -Path $installLocation
				Remove-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\nbi-nb-base-8.1.0.0.201510222201'
				Remove-RegistryKey -Key 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\nbi-nb-base-8.1.0.0.201510222201-1' -ContinueOnError $True
				Remove-Folder -Path "${envCommonStartMenuPrograms}\NetBeans"
			} Else {
				Write-Log -Message "An unknown version of the application was found and hasn't been removed. Continuing to installation..." -Source $deployAppScriptFriendlyName
			}
		} Else {
			Write-Log -Message "A previous installation not found. Continuing to installation..." -Source $deployAppScriptFriendlyName
		}
		#>
		##*===============================================
		##* INSTALLATION
  	...
  	$exitCode = Execute-Process -Path "$dirFiles/Apache-NetBeans-11.0-bin-windows-x64.exe" -Parameters "--silent" -WindowStyle "Hidden" -WaitForMsiExec -PassThru
		...
		$exitCode = Execute-MSI -Action 'Uninstall' -Path '{4A03706F-666A-4037-7777-5F2748764D10}' -PassThru
		Remove-File -Path "${envCommonDesktop}\NetBeans*.lnk" -ContinueOnError $true
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'iexplore,firefox,netbeans64' -CloseAppsCountdown 60
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBM2LiNLKAn/FPI
# /ys/50EBdQzM/jOacIptui894nhaEKCCFFwwggQUMIIC/KADAgECAgsEAAAAAAEv
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgHCWaKOrt
# QV8YkJfwshu/YKVAHn/O5X/oJX+izSbGKTYwDQYJKoZIhvcNAQEBBQAEggEAKCwl
# 3gJUafNT0Xiv5dRp5ZDPLHAvfz4ZpP6xUqllELj0XuglQK/dVYG2oXsznY85tCNx
# bqhwpqoB5qmVFLWbMShkvDakxlKrsbDAJvksVGidBs0i5YSVnav6DAzNV+nFl5NB
# ULfw/Jwm7DHYetweZeV50L50cjg/idCVr4VegGBW+1XxgXAfLT76iLe2ihc5SR7b
# G8727t4Et8ieuZuScwzsuZQi+bB4Tg0rDBbW9SOCox0Bo6MSCjREpImH1CgrJCmk
# W+ipHbZRqcHdS5QU8lkorykdEDoI4BO6KV5u2lf9U7aleKH0YDelhQ0646xWOfcu
# Qz0dlPZQZ0JPn6um66GCAqIwggKeBgkqhkiG9w0BCQYxggKPMIICiwIBATBoMFIx
...
# CSqGSIb3DQEJBTEPFw0xOTA3MjcxNzAzNTJaMCMGCSqGSIb3DQEJBDEWBBQISP8q
# IJZWzAjQvZXZdC2w5KQ5ODCBnQYLKoZIhvcNAQkQAgwxgY0wgYowgYcwgYQEFGO4
...
# ggEAOiOw/BcHDogbXfpC0lrzGDVDnELvs6TyU5OfK8zh0QQcnl2fRdym0CAL3DFg
# nL/NQ60U2SiRs5qnY4H99Caf2gzc559QVCa7r69atQCSwI5+znACsZUDEDg3nCoH
# URIGN4Ef52fEdpYfIXLrHbtEbrP76/Td1DIKOotG1Ti/CR65+QrB1AJwaNKY0Eay
# b6pfPYZ4GukykIIj3+uZoALsNrtjGVv38mZjpt+8mG2rj+UUIHx+yZuSVdFgJChz
# HBjt1kVydrm3+PM5/hSxFlLro0if3A/6D6XCN0FGgObTMi0BPSfxdzAY+Z8VolaW
# CHSIBnQQhSD71mWCLlqStos7HQ==
